...
(function () {
	...
	*/
	...
	gui.showQueryResult = function (point, layer, obj, show_coords) {
		// Add: Hide the graph to avoid overlapped data
		...
		// Add: API call and graph response
		// Create the canvas from zero to avoid overlaps
		document.getElementById("canvas-container").innerHTML =
			'<canvas id="PLUVIOMETER"></canvas>';
		switch (layer.properties.name) {
			case "PIEDICAVALLO_Pluviometer":
				getPtotDataForDate("PIE-002095-900", "PIEDICAVALLO").then(
					(response) => {
						if (response !== null) {
							// Save the response from API call
							// var responseData = response;
							// Method that created the graph starting from the response
							plotPtotDataForDate(response);
						}
					}
				);
				break;
			case "BIELLA_Pluviometer":
				getPtotDataForDate("PIE-002012-901", "BIELLA").then((response) => {
					if (response !== null) {
						// Save the response from API call
						// var responseData = response;
						// Method that created the graph starting from the response
				...
				break;
			default:
				// Clean the canvas in case other points are selected
				document.getElementById("canvas-container").innerHTML = "";
				break;
		}
...
// Add: The Pluviometer.js file used for the 2D web map is here inserted 
...
async function plotPtotDataForDate(res) {
	...
	document.getElementById("PLUVIOMETER").style.display = "block";
	var ctx = document.getElementById("PLUVIOMETER").getContext("2d");
